...
def extract_text_from_docx(file_path):
    """Extract paragraph and table text from a .docx policy document."""
    ...
    # python-docx does not include table cells in Document.paragraphs. Policy
    # thresholds and eligibility matrices are commonly stored in tables.
...
def main():
    ...
    for filename in sorted(os.listdir(docs_dir)):
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                # Stable IDs make ingestion idempotent; reruns update instead of duplicating chunks.
                uid = f"{target}:{filename}:{i}"
                if target == "loan_docs":
    ...
    print("\nAll done!")
    print(f"Final collection counts: policy_docs={policy_rag.collection.count()}, loan_docs={loan_rag.collection.count()}")
