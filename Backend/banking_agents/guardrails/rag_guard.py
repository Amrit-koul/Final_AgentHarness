...
class RAGGuard:
    ...
    def check(self, retrieved_results: dict, session_id: str = "") -> tuple[bool, str | None]:
        """
        ...
        if not retrieved_results or not retrieved_results.get("documents") or not retrieved_results["documents"][0]:
            ...
            emit_guardrail_event("rag.no_results", "No documents were retrieved", session_id)
        ...
        if not distances:
            ...
            emit_guardrail_event("rag.no_distances", "Retrieval returned no relevance distances", session_id)
            return False, self.no_result_message
        ...
        if min_dist > self.hard_threshold:
            ...
            emit_guardrail_event(
                "rag.hard_threshold",
                f"Best retrieval distance {min_dist:.4f} exceeded {self.hard_threshold:.4f}",
                session_id,
            )
            return False, self.no_result_message
        ...
        if min_dist > self.soft_threshold:
            ...
            emit_guardrail_event(
                "rag.soft_threshold",
                f"Best retrieval distance {min_dist:.4f} exceeded {self.soft_threshold:.4f}",
                session_id,
            )
