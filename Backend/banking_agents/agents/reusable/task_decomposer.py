...
class TaskDecomposerAgent:
    ...
    def decompose(self, query: str, intent: str) -> list[str]:
        ...
        loaded = prompt_registry.load("task_decomposer")
        developer_prompt = Template(loaded.developer).safe_substitute(intent=intent)
        sections = [loaded.system.strip(), developer_prompt.strip()]
        if loaded.few_shots.strip():
            sections.extend(["# Examples", loaded.few_shots.strip()])
